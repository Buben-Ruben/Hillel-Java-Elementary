...
public class MyLinkedList<E> implements List<E>,Iterable<E>,MyListIterator {
    ...
    private static class Node<E> {
        ...
        public Node(Object currentElement) {
        }
    ...
    public E remove(int index) {
        Node<E> node = HEADER.nextElement;
        int i = 0;
        if (0<=index && index<size){
            while (node!=null && node!=HEADER){
                if (i == index){
                    node.prevElement = node.nextElement;
                    node.nextElement = node.prevElement;
